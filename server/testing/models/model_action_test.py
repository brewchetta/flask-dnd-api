...
class TestAction:
    """ [TESTING SUITE: <Action>] """

    def test_Action_has_attributes(self):
        """ (attributes) Has proper attributes """
        s = Action(name="Stabby Stab", description="Stabs with a dagger", monster_id=1)
        db.session.add(s)
        db.session.commit()

        db_item = Action.query.first()
        assert db_item
